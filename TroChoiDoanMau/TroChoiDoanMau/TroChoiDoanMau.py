...
def hien_thi_game_over(secret_colors, round_size):
    screen.fill(BLACK)
...
def vong_lap_trong_tro_choi():
    ...
    while running:
        ...
        if attempts >= max_attempts:
            hien_thi_game_over(secret_colors, round_sizes[round_index])
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # Kiểm tra nhấn vào các vòng màu
                x = 20
                y = 215  # Vị trí bắt đầu của các vòng màu
                spacing = 70
                for i in range(round_sizes[round_index]):
                    # Nếu nhấn vào vòng màu, phát âm thanh
                    if x <= event.pos[0] <= x + 40 and y - 20 <= event.pos[1] <= y + 40:
...
def huong_dan():
    ...
    while running:
        ...
        - Quan sát kỹ phản hồi của máy sau mỗi lần đoán.
        - Hãy cố gắng tìm ra mẫu hình và dựa vào đó để tiếp tục đoán màu sắc chính xác.
        - Sử dụng các chiến thuật thử nghiệm và điều chỉnh khi cần thiết.
...
def ve_menu(selected_index):
    ...
    for i, (item, description) in enumerate(menu_items):
        ...
        button_text = font_button.render(item, True, WHITE)
        screen.blit(button_text, (400 - button_text.get_width() // 2, 330 + i * 60))

    # Mô tả cho mục được chọn
    description_text = font_text.render(menu_items[selected_index][1], True, WHITE)
